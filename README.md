...
webpack 中 output 的 publicPath 需与 webpack-dev-server 的 publicPath 一直，不若，引入拆分的资源会因为路径问题报错。例如 `/aa/list` 可能会是引用 `/aa/static/aa.xx.js`，其实应该是 `/static` 路径才对。
...
antd 与 css-module 冲突了，写了两个解析规则，互相屏蔽才搞定。